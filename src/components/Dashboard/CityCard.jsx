...
function CityCard() {
      ...
      try {
        cities &&
          cities.map(async (cityCode) => {
            await axios
              .get(
                `https://api.openweathermap.org/data/2.5/weather?id=${cityCode}&appid=1fc7bb3360d9a0dc1ee6edf72b006c7c`
              )
              .then((res) => {
  ...
  return ( 
    <>
      {newforecast &&
              newforecast.map((item) => {
                  return <h1 key={item.id} className="text-white text-xl"> {item.name}</h1>
              })}
    </>
    ...
    //                <div>
    //                       {newforecast && <div className='text-white font-medium text-lg'>{newforecast.length},LK</div>}
